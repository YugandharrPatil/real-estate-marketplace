...
import { bookVisitAction, toggleSaveAction } from "@/actions/user";
...
import { Bath, BedDouble, Calendar, ChevronDownIcon, ChevronLeft, ChevronRight, Heart, MapPin, Maximize, MessageSquare, Share2 } from "lucide-react";
...
export function PropertyDetail({ property, isSaved: initialSaved, isLoggedIn }: PropertyDetailProps) {
	const { user } = useUser();
			...
			<div className="relative aspect-[16/9] md:aspect-[21/9] rounded-xl overflow-hidden bg-muted">
				{images.length > 0 ? (
					<>
						...
						{images.length > 1 && (
							<>
								<button onClick={() => setCurrentImage((currentImage - 1 + images.length) % images.length)} className="absolute left-3 top-1/2 -translate-y-1/2 bg-background/80 backdrop-blur rounded-full p-2 hover:bg-background transition">
									<ChevronLeft className="h-5 w-5" />
								</button>
								<button onClick={() => setCurrentImage((currentImage + 1) % images.length)} className="absolute right-3 top-1/2 -translate-y-1/2 bg-background/80 backdrop-blur rounded-full p-2 hover:bg-background transition">
									<ChevronRight className="h-5 w-5" />
								</button>
								<div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
									{images.map((_, i) => (
										<button key={i} onClick={() => setCurrentImage(i)} className={`h-2 w-2 rounded-full transition ${i === currentImage ? "bg-white" : "bg-white/40 hover:bg-white/60"}`} />
									))}
								</div>
							</>
						)}
					...
					<div className="flex items-start justify-between">
						<div className="space-y-2">
							<div className="flex items-center gap-2">
								...
								<Badge variant={property.status === "available" ? "default" : property.status === "pending" ? "secondary" : "destructive"}>{property.status}</Badge>
							</div>
									...
									<FieldGroup className="flex-row">
												...
												render={({ field }) => (
													<Popover open={datePickerOpen} onOpenChange={setDatePickerOpen}>
														<PopoverTrigger asChild>
															<Button type="button" variant="outline" id="visit-date" className="w-full justify-between font-normal">
										...
										<Field data-invalid={!!errors.visitTime}>
											...
											<Controller
												name="visitTime"
												control={control}
												render={({ field }) => <Input type="time" id="visit-time" {...field} className="appearance-none bg-background [&::-webkit-calendar-picker-indicator]:hidden [&::-webkit-calendar-picker-indicator]:appearance-none" />}
											/>
