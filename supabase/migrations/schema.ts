import { pgTable, foreignKey, uuid, text, date, timestamp, numeric, integer, boolean, pgSequence, pgEnum } from "drizzle-orm/pg-core"
...
export const reVisits = pgTable("re_visits", {
...
export const reSavedProperties = pgTable("re_saved_properties", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	propertyId: uuid("property_id").notNull(),
	userId: text("user_id").notNull(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
...
export const reChats = pgTable("re_chats", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	userId: text("user_id").notNull(),
	userName: text("user_name").notNull(),
	userEmail: text("user_email").notNull(),
...
export const reMessages = pgTable("re_messages", {
	id: uuid().defaultRandom().primaryKey().notNull(),
...
export const reInquiries = pgTable("re_inquiries", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	userId: text("user_id"),
	name: text().notNull(),
	email: text().notNull(),
